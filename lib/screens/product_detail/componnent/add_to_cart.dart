  ...
  return BlocConsumer<HomeCubit, AppCubitStates>(
      builder: (context, state) => Column(
        ...
        children: [
                ...
                BlocConsumer<AppCubit, AppCubitStates>(
                  builder: (context, state) {
                    return DropdownButton<String>(
                      value:  AppCubit.get(context).sizeTitleselected,
                      icon: Icon(Icons.keyboard_arrow_down, color: Colors.black, size: w * 0.08),
                      underline: Container(),
                      ...
                      onChanged: (value) async {
                        int selectedIndex = size.indexWhere((sizeItem) => sizeItem.name == value);
                        SharedPreferences prefs = await SharedPreferences.getInstance();
                        prefs.setString('size_id', size[selectedIndex].id.toString());
                        prefs.setString('sizeOption', size[selectedIndex].pivot!.id.toString());

                        //   setState(() {
                        //    selectedSizeName = value!;
                        AppCubit.get(context).sizeTitleselected = value;
                        //  });

                        AppCubit.get(context).sizeSelection(
                          selected: selectedIndex,
                          title: value,
                        );

                        HomeCubit.get(context).getProductColor(
                          context: context,
                          productId: productId,
                          sizeId: size[selectedIndex].id.toString(),
                        );
                      },
                    );
                  },
          ...
          //   ),
          // ),
          BlocBuilder<HomeCubit, AppCubitStates>(
              builder: (context, state) {
                                ...
                                builder: (context, state) {
                                  return
                                    DropdownButton<String>(
                                      value: AppCubit.get(context).colorTitleselected,
                                     icon: Icon(Icons.keyboard_arrow_down, color: Colors.black, size: w * 0.08),
                                      underline: Container(),
                                      ...
                                      onChanged: (value) async {
                                        int selectedIndex = HomeCubit.get(context)
                                            .singleProductColorModel!
                                            .data!
                                            .indexWhere((colorItem) => colorItem.name == value);

                                        SharedPreferences prefs = await SharedPreferences.getInstance();
                                        prefs.setString('color_id', HomeCubit.get(context)
                                            .singleProductColorModel!
                                            .data![selectedIndex]
                                            .heightId
                                            .toString());
                                        prefs.setString('colorOption', HomeCubit.get(context)
                                            .singleProductColorModel!
                                            .data![selectedIndex]
                                            .id
                                            .toString());
                                        ...
                                        AppCubit.get(context).colorTitleselected = value;
                                        // });

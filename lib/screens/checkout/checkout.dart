...
class ConfirmCart extends StatefulWidget {
  ...
  const ConfirmCart(
      ...
      required this.city,
      required this.address, required this.name, required this.email, required this.phone})
      : super(key: key);
...
class _ConfirmCartState extends State<ConfirmCart> {
  ...
  Future<void> createSession() async {
    ...
    if(islogin){
      if(OrderCubit.get(context).allOrdersModel!.orders!.isNotEmpty){
        for(var i =0 ;i< OrderCubit.get(context).allOrdersModel!.orders!.length ;i++){
          if(OrderCubit.get(context).allOrdersModel!.orders![i].status == 1){
            setState(() {
              loyalty++;
            });
          }
    ...
    try {
      ...
      final s = await TabbySDK().createSession(TabbyCheckoutPayload(
        merchantCode: currency == 'BHD' ?  "rayanstorebhr" : currency == 'QAR' ?"rayanstoreqat" : currency == 'AED' ? "rayanstoreare" : currency == 'SAR' ?  "rayanstoresau" : "rayanstorekwt",
        lang: lang == 'en' ? Lang.en : Lang.ar,
        payment: Payment(
          amount: getProductPriceTabby(currency: currency, productPrice: num.parse(widget.totalPrice)),
          ...
          orderHistory: islogin ? (OrderCubit.get(context)
              .allOrdersModel!
              .orders!
              .isNotEmpty)
              ? List.generate(OrderCubit.get(context).allOrdersModel!.orders!.length, (index) => OrderHistoryItem(amount: getProductPriceTabby(currency: currency, productPrice: num.parse(OrderCubit.get(context).allOrdersModel!.orders![index].totalPrice!)), status: OrderCubit.get(context).allOrdersModel!.orders![index].status == 2 ? OrderHistoryItemStatus.complete : OrderCubit.get(context).allOrdersModel!.orders![index].status == 1 ? OrderHistoryItemStatus.processing : OrderHistoryItemStatus.newOne, purchasedAt: OrderCubit.get(context).allOrdersModel!.orders![index].createdAt!)) :
          [
            OrderHistoryItem(
  ...
  void initState() {
    ...
    context.read<AppCubit>().paidBy= LocalKeys.VISA.tr();
    super.initState();
  }
              ...
              children: [
                ...
                children: [
                  ...
                  InkWell(
                    onTap: () {
                      setState(() {
                        _counter = 3;
                        context.read<AppCubit>().paidBy=                             "الدفع عن طريق tappy"
                        ;
                      });
                      createSession();
                    },
                  ...
                  if(BlocProvider.of<AppCubit>(context).tabbyStatusValue == true && _counter == 3)
                    Padding(
                  ...
                  if(_counter == 3)
                    TabbyCheckoutSnippet(price: getProductPriceTabby(currency: currency, productPrice: num.parse(widget.totalPrice)),
                      currency: currency == 'BHD' ? Currency.bhd : currency == 'QAR' ? Currency.qar : currency == 'AED' ? Currency.aed : currency == 'SAR' ? Currency.sar : Currency.kwd,
                ...
                InkWell(
                  onTap: () {
                    setState(() {
                      _counter = 1;
                      context.read<AppCubit>().paidBy= translateString(
                                      ...
                                      builder: (context, state) {
                                        return Row(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              "${CartCubit.get(context).deliveryModel!.delivery} ${LocalKeys.DAYS.tr()}",
                                              style: TextStyle(
                                                  color: Colors.black,
                                                  fontFamily: (lang == 'en')
                                                      ? 'Nunito'
